...
const getStringbyAlpha2Code = (alpha2Code) =>
  contriesArray
    .filter((el) => el.alpha2Code === alpha2Code)
    .forEach((el) => {
      return console.log(
        `El país es ${el.name}, su población es de ${el.population} habitantes y su área total es de ${el.area} km2`
      );
    });
console.log(getStringbyAlpha2Code("CN"));

// Crear una función que permita ingresar el alpha2Code de dos países, y me indique cuál de los dos países tiene mayor densidad de población ( cantidad de habitantes / km2 )
const getHigherDensity = (a2c1, a2c2) => {
  let dContry1 =
    contriesArray.find((el) => el.alpha2Code === a2c1).population /
    contriesArray.find((el) => el.alpha2Code === a2c1).area;
  let dCotry2 =
    contriesArray.find((el) => el.alpha2Code === a2c2).population /
    contriesArray.find((el) => el.alpha2Code === a2c2).area;
  let result =
    dContry1 > dCotry2
      ? `El país con mayor densidad de población entre ${
          contriesArray.find((el) => el.alpha2Code === a2c2).name
        } y ${contriesArray.find((el) => el.alpha2Code === a2c1).name} es ${
          contriesArray.find((el) => el.alpha2Code === a2c1).name
        } con ${dContry1} habitantes por metro cuadrado`
      : `El país con mayor densidad de población entre ${
          contriesArray.find((el) => el.alpha2Code === a2c1).name
        }  y ${contriesArray.find((el) => el.alpha2Code === a2c2).name}  es ${
          contriesArray.find((el) => el.alpha2Code === a2c2).name
        } 
        } con ${dContry1} habitantes por metro cuadrado`;
  return result;
};
console.log(getHigherDensity("CN", "MX"));

// -Crear una función que me permita saber cuál es el país de la lista con mayor población y cuál es el país con menor población
const getSmallerAndhHigherPupulation = () => {
  let higher = 0;
  let smaller = 0;
  contriesArray.reduce((acc, cur) =>
    acc > cur.population ? (higher = acc) : (higher = cur.population)
  );
  contriesArray.reduce((acc, cur) =>
    acc < cur.population ? (smaller = acc) : (smaller = cur.population)
  );
  return `Menor poblacion : ${smaller}  Mayor poblacion: ${higher}`;
};
console.log(getSmallerAndhHigherPupulation());
